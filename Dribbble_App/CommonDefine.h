//
//  CommonDefine.h
//  Dribbble_App
//
//  Created by oda yuma on 2014/08/09.
//  Copyright (c) 2014年 yuma oda. All rights reserved.
//

#ifndef Dribbble_App_CommonDefine_h
#define Dribbble_App_CommonDefine_h

// API
#define API_DRIBBBLE_DOMAIN     @"http://api.dribbble.com/shots/"
#define API_DRIBBBLE_EVERYONE   @"everyone"
#define API_DRIBBBLE_DEBUTS     @"debuts"
#define API_DRIBBBLE_POPULAR    @"popular"

// color
#define COLOR_BACKGROUND        [UIColor colorWithRed:255.0/255.0 green:235.0/255.0 blue:205.0/255.0 alpha:1]

#endif
